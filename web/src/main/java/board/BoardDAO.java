...
//			String sql="select * from board order by num desc";
//			String sql="select * from board order by num desc limit 시작하는 행번호-1,한페이지 보여줄글개수";
			String sql="select * from board where content like ? order by num desc limit ?,?";
			pstmt=con.prepareStatement(sql);
...
//				System.out.println(boardDTO);
	...
	public List<BoardDTO> getBoardList1(int startRow,int pageSize, String ddate) {
		Connection con=null;
		PreparedStatement  pstmt=null;
		ResultSet rs=null;
		// List 객체생성(BoardDTO 여러개를 담을수 있는 자바 배열형태의 내장객체)
...
//			String sql="select * from board order by num desc";
//			String sql="select * from board order by num desc limit 시작하는 행번호-1,한페이지 보여줄글개수";
			String sql="select * from board where date like ? order by num desc limit ?,?";
			pstmt=con.prepareStatement(sql);
...
//				System.out.println(boardDTO);
	...
	public int getBoardCount1(String ddate) {
		int count=0;
		Connection con=null;
		PreparedStatement  pstmt=null;
		ResultSet rs=null;
		try {
			...
			con=getConnection();
			// 3 sql select 게시판 num 글 가져오기
			String sql="select count(*) from board where date like ? ";
			pstmt=con.prepareStatement(sql);
			pstmt.setString(1, ddate +"%");
			// 4 실행 => 결과 저장
			rs=pstmt.executeQuery();
