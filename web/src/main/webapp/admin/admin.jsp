...
List<BoardDTO> boardList=boardDAO.getBoardList1(startRow, pageSize, ddate);
...
//     	BoardDTO boardDTO=(BoardDTO)boardList.get(i);
    	...
    	%>
 <tr onclick="location.href='../center/content.jsp?num=<%=boardDTO.getNum()%>'"><td><%=boardDTO.getNum() %></td>
...
int count=boardDAO.getBoardCount1(ddate);
...
if(startPage > pageBlock){
	%><a href="admin.jsp?pageNum=<%=startPage-pageBlock %>">Prev</a><%
}
...
for(int i=startPage;i<=endPage;i++){
	if(i==currentPage){
		...
		%>
		<a href="admin.jsp?pageNum=<%=i %>&ddate=<%=ddate %>"><%=i %></a>
		<%
	}
...
if(endPage < pageCount){
	%>
	<a href="admin.jsp?pageNum=<%=startPage+pageBlock %>">Next</a>
	<%
}
%>
